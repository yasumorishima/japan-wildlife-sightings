...
27 の公開元から **79,394 件**を取得できています（うち座標つき 75,743 件）。
**石川県はカタログ経由と Google マイマップ経由の 2 本を取得しているので、この数字では同じ出没を二重に数えています**（`data/` に収録するのはカタログ経由だけです）。
このうち **40,241 件**を `data/` に収録しています。差は、**公開元の規約で再配布が
...
| 福島県 クマ目撃マップ | ArcGIS | 4,493 | — |
| 石川県 クマ出没データ | 県の CKAN の CSV | 2,701 | ✅ |
| 石川県（別経路） | Google マイマップ | 3,659 | — |
| 石川県 金沢市 ツキノワグマ目撃痕跡（座標なし） | 県の CKAN の CSV | 754 | ✅ |
| 石川県 金沢市 イノシシ出没（座標なし） | 県の CKAN の CSV | 69 | ✅ |
| 石川県 金沢市 サル出没（座標なし） | 県の CKAN の CSV | 46 | ✅ |
| 宮城県 | Google マイマップ | 3,535 | — |
| 群馬県 | ArcGIS Survey123 | 1,736 | — |
| 山形県 | CSV | 869 | — |
| 静岡県（2 地図） | Google マイマップ | 475 | — |
| 埼玉県 | ArcGIS Survey123 | 362 | — |
| 富山県 クマっぷ | ArcGIS Survey123 | 4,614 | — |
| 三重県 | ArcGIS | 113 | — |
| 北海道 札幌市 ヒグマ出没情報 | 市の CKAN の CSV（2017〜） | 1,568 | ✅ |
| 北海道 室蘭市 | ArcGIS Survey123 | 14 | ✅ |
| 北海道 登別市 | ArcGIS | 68 | — |
| 北海道 石狩市 | ArcGIS | 214 | ✅ |
| 北海道 上砂川町 | BODIK（CKAN） | 98 | ✅ |
| 山口県 熊の目撃情報（県警察認知） | 県独自 CKAN の CSV | 1,230 | ✅ |
| 長野県（月別 PDF・座標なし） | PDF | 2,026 | — |
| 山梨県（年度別 PDF・座標なし） | PDF（座標つき抽出） | 751 | — |
| 長野県 岡谷市 | ArcGIS | 46 | — |

## まだ取れていないもの

**クマが生息する 40 都道府県のうち、点データを機械可読で出しているのは 14 です。**
...
長野県ぶんは `pdf` アダプタで読めるようになりました（22 本の月別 PDF から 2,026 件）。
...
同じ URL でも、**どこから取りに行くかで結果が変わります**。

- **石川県のカタログ（`ckan.opendata.pref.ishikawa.lg.jp`）は GitHub のランナーから 403**
  になります。User-Agent を変えてもトップページごと拒否されるので、IP による遮断です
  （応答は `server: awselb/2.0`）。日本国内の回線からは 200 で取得できます。
- **山梨県のサイトは GitHub のランナーから接続タイムアウト**します。
- 山口県のカタログは**旧式の TLS しか持たない**ため、ランナーの既定の暗号方針では
  サーバ側が接続を拒否します（暗号の要求だけを緩めて取得しています。証明書の検証は緩めません）。

そのため、**収録している出典が取得できなかった回は、前回のデータをそのまま残します**。
1 つの公開元が届かないだけで、収録済みの数千件が消えるほうが害が大きいからです。
据え置いたことは `data/summary.json` の `reused_previous` と実行ログに必ず出ます。
...
**収録しているのは 11 出典**で、うち**座標を持つ 8 出典が地図に出ます**
（秋田・京都・山口・石川・札幌市・室蘭市・石狩市・上砂川町。金沢市の 3 出典は町名までで座標がありません）。
